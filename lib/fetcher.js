...
exports.createExif = (pack, auth) =>{
    ...
    const exif = {"sticker-pack-id": "com.leo.tech", "sticker-pack-name": pack, "sticker-pack-publisher": auth, "android-app-store-link": "https://play.google.com/store/apps/details?id=com.termux", "ios-app-store-link": "https://itunes.apple.com/app/sticker-maker-studio/id1443326857"}
...
exports.modStick = (media, leo, choute, from) => {
    out = getRandom('.webp')
    try {
        console.log(media)
        spawn('webpmux', ['-set','exif', './stik/data.exif', media, '-o', out])
        .on('exit', () => {
            leo.sendMessage(from, fs.readFileSync(out), 'stickerMessage', {quoted: choute})
            fs.unlinkSync(out)
            fs.unlinkSync(media)
        })
    } catch (e) {
        console.log(e)
        leo.sendMessage(from, 'Terjadi keslahan', 'conversation', { quoted: choute })
        fs.unlinkSync(media)
    }
}
